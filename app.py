...
#     "Solar":("12 solar-energy-consumtion.csv","Electrcity from solar (TWh)"),
...
#     data = load_data()
#     plt.subplots(figsize=(3,2))
#     df = pd.DataFrame(list(data.items()), columns=['Fuente','Produccion (TWh)'])

#     fig, ax= plt.subplots(figsize=(5,4))
#     ax.bar(df['Fuente'], df['Producción (TWh)'], color=['blue','orange','green','red','purple'])

#     ax.set_xtitle('Producción de Energía Renovable por Fuente', fontsize = 12)
#     ax.set_ylabel('Produccion (TWh)',fontsize=12)
...
#     return render_template('index.html',porcentaje_renovable = porcentaje_renovable)

# Definición de la función obtener_porcentaje_renovable
def obtener_porcentaje_renovable():
    # Suponiendo que es un valor calculado o extraído de algún lado
    return 45.67  # Ejemplo de un valor fijo

@app.route('/')
def index():
    porcentaje_renovable = obtener_porcentaje_renovable()  # Llamamos a la función
    return render_template('index.html', porcentaje_renovable=porcentaje_renovable)
